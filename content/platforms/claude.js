// Claude Prompt Timeline - Simple sidebar that shows prompts and scrolls to them
...
class PromptTimeline {
  ...
  async init() {
    console.log('🛤️ PromptTrail: Initializing for Claude...');
  ...
  waitForConversation() {
    return new Promise((resolve) => {
      const check = () => {
        ...
        // Check if any user messages are loaded (Claude-specific selector)
        const hasMessages = document.querySelector('[data-testid="user-message"]');
        if (main && hasMessages) {
          resolve();
        } else if (main) {
  ...
  getConversationId() {
    // Extract conversation ID from Claude URL: /chat/abc123-def456-...
    const match = window.location.pathname.match(/\/chat\/([a-zA-Z0-9-]+)/);
    return match ? match[1] : 'default';
  }
  
  getStorageKey() {
    return `prompttrail-claude-bookmarks-${this.conversationId}`;
  }
  ...
  loadSidebarWidth() {
    try {
      const saved = localStorage.getItem('prompttrail-claude-sidebar-width');
      if (saved) {
  ...
  saveSidebarWidth() {
    try {
      localStorage.setItem('prompttrail-claude-sidebar-width', this.sidebarWidth.toString());
    } catch (e) {
  ...
  getStableMessageId(element) {
    // Try to get a stable ID for Claude messages
    
    // 1. Check for data-testid on the element itself
    const testId = element.getAttribute('data-testid');
    if (testId && testId !== 'user-message') {
      return testId;
    }
    
    // 2. Look for parent with data-message-id or similar
    const parentWithId = element.closest('[data-message-id]');
    if (parentWithId) {
      const messageId = parentWithId.getAttribute('data-message-id');
      if (messageId) return messageId;
    }
    
    // 3. Look for conversation turn wrapper
    const turnWrapper = element.closest('[data-testid^="conversation-turn"]');
    if (turnWrapper) {
      const turnId = turnWrapper.getAttribute('data-testid');
  ...
  scanExistingPrompts() {
    // Claude-specific selector for user messages
    const userMessages = document.querySelectorAll('[data-testid="user-message"]');
    
    console.log('🛤️ PromptTrail: Found', userMessages.length, 'existing prompts');
  ...
  addPromptFromElement(element, index = null) {
    ...
    // Extract text from Claude's user message structure
    // Claude uses <p> tags inside the user-message div
    const textEl = element.querySelector('p') || element;
    ...
    console.log('🛤️ PromptTrail: Added prompt:', text.substring(0, 50) + '...');
    return prompt;
  }
      ...
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            // Check if this node or its children contain a user message (Claude-specific)
            const userMsg = node.matches?.('[data-testid="user-message"]') 
              ? node 
  ...
  copyToInput(promptId) {
    ...
    // Find Claude's input - it uses contenteditable divs
    ...
    if (!input) {
      console.warn('🛤️ PromptTrail: Could not find Claude input');
    ...
    if (input.tagName === 'TEXTAREA') {
      ...
      // Contenteditable div (Claude uses ProseMirror)
      input.innerHTML = '';
...
// Handle SPA navigation (Claude is a single-page app)
